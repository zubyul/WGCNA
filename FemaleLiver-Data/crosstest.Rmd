...
for(i in names(test.volume[,6:19])){
  var <- paste(i)
  lm <- summary(glm(test.volume[[i]]~test.volume$male.weighted+test.volume$Age_in_Yrs +test.volume$FS_IntraCranial_Vol))
  tval <- lm$coefficients[2,3]
  pval <- lm$coefficients[2,4]
  tval.volume[[i]] <- assign(var,round(tval,3))
  pval.volume[[i]] <- assign(var,round(pval,3))}  #test.volume$Gender
...
for(i in names(test.thickness[5:72])){
  var <- paste(i)
  lm <- summary(glm(test.thickness[[i]]~test.thickness$male.weighted+test.thickness$Age_in_Yrs
                    #+test.thickness$Gender
                    ))
  tval <- lm$coefficients[2,3]
  pval <- lm$coefficients[2,4]
...
for(i in names(test.surfarea[,6:73])){
  var <- paste(i)
  lm <- summary(glm(test.surfarea[[i]]~test.surfarea$male.weighted+test.surfarea$Age_in_Yrs+
                      #test.surfarea$Gender
...
write.csv(out.surfarea,'out_SA_M_MPRS.csv')
write.csv(out.thickness,'out_thick_M_MPRS.csv')
write.csv(out.volume,'out_vol_M_MPRS.csv')
```
...
colnames(genetic)[2] <- "SID"
#3MW 4FW 5MU 6FU          \/
data <- merge(genetic[,c(2,4)],clinical[,c(1,2,10,11,204)],by.x='SID',by.y='Subject')
data <- merge(data,imaging,by.x='SID',by.y='Subject')
...
for(i in names(test.volume[,6:19])){
  var <- paste(i)
  lm <- summary(glm(test.volume[[i]]~test.volume$female.weighted+test.volume$Age_in_Yrs+test.volume$FS_IntraCranial_Vol)) #+test.volume$Gender
  tval <- lm$coefficients[2,3]
  pval <- lm$coefficients[2,4]
  tval.volume[[i]] <- assign(var,round(tval,3))
...
for(i in names(test.thickness[5:72])){
  var <- paste(i)
  lm <- summary(glm(test.thickness[[i]]~test.thickness$female.weighted+test.thickness$Age_in_Yrs #+test.thickness$Gender
                    ))
  tval <- lm$coefficients[2,3]
  pval <- lm$coefficients[2,4]
  tval.thickness[[i]] <- assign(var,round(tval,3))
...
for(i in names(test.surfarea[,6:73])){
  var <- paste(i)
  lm <- summary(glm(test.surfarea[[i]]~test.surfarea$female.weighted+test.surfarea$Age_in_Yrs+test.surfarea$FS_IntraCranial_Vol)) #test.surfarea$Gender+
  tval <- lm$coefficients[2,3]
  pval <- lm$coefficients[2,4]
  tval.surfarea[[i]] <- assign(var,round(tval,3))
  pval.surfarea[[i]] <- assign(var,round(pval,3))}
...
write.csv(out.surfarea,'out_SA_F_FPRS.csv')
write.csv(out.thickness,'out_thick_F_FPRS.csv')
write.csv(out.volume,'out_vol_F_FPRS.csv')

```


Female score on male data
```{r}
library(ggplot2)
...
write.csv(out.surfarea,'out_SA_M_FPRS.csv')
write.csv(out.thickness,'out_thick_M_FPRS.csv')
write.csv(out.volume,'out_vol_M_FPRS.csv')
...
write.csv(out.surfarea,'out_SA_F_MPRS.csv')
write.csv(out.thickness,'out_thick_F_MPRS.csv')
write.csv(out.volume,'out_vol_F_MPRS.csv')
```